...
const HoveredPreview = ({movie, unhover, show, changeIconState}:props)=>{
    const[hoveredIcons, setHoveredIcons] = useState<HoveredIcons>({
        play:false,
        liked:false
    })

    const handleHover = (name:'liked'|'play') =>{
        setHoveredIcons({...hoveredIcons, [name]:true})
    }

    const handleUnhover = (name:'liked'|'play') => {
        setHoveredIcons({...hoveredIcons, [name]:false})
    }

    const toggleIcon = (name:'added'|'liked')=>{
