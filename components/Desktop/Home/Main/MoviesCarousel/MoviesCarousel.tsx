import MyMovieModel, {MyMovie} from '@/backend/models/MyMovie'
...
import { MovieType } from './Dropdown/Dropdown'
...
const MovieCarousel = ({myMovies, popularMovies}:props) => {
    ...
    const setMoviesArray = (type:MovieType)=>{
    ...
    const changeIconState = (title: string, name:string, value:boolean)=>{
        setPopularMoviesState(popularMoviesState.map(movie=>movie.title === title? {...movie, [name]:value}:movie))
    ...
    return( 
        <CarouselWrapper>
            ...
            <MoviesWrapper>
                    {
                    showMyMovies?
                    movies.slice(movieOffset, movieOffset+4).map((movie:MyMovie | Movie, index:number)=><MyMoviePreview key={index} movie={movie as MyMovie}/>):
                    movies.slice(movieOffset, movieOffset+4).map((movie:Movie | MyMovie, index:number)=><MoviePreview key={index} changeIconState={changeIconState}movie={movie as Movie} />)
                    }
            </MoviesWrapper>
