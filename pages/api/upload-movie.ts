import { type NextConnectApiRequest, type ResponseData } from 'backend/handlers/types'
...
handler.post(async (req: NextConnectApiRequest, res: NextApiResponse<ResponseData>) => {
  ...
  try {
    ...
    if (newMovie === undefined) res.status(400).json({ data: { message: 'Could not add movie, database document creation failed' } })
  } catch (error) {
    console.log(error)
  }
  connection.close()
